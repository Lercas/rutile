.PHONY: help build install test test-linux lint security smoke release-check clean site site-audit
...
release-check: lint security test smoke test-linux site-audit site ## Everything that must be green for a release
	@echo "release-check passed for $(VERSION)"

clean: ## Remove build artifacts
	rm -f $(BINARY)

site: ## Build the landing page (site/dist)
	cd site && npm run build && npm run validate

site-audit: ## Check landing-page production dependencies
